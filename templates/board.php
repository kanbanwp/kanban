<?php include Kanban_Template::find_template( 'inc/header' ) ?>
	...
	<div id="wrapper-footer">
		...
		<div class="form-inline" id="search-wrapper">
			<input type="search" id="board-search" class="form-control"
			       placeholder="<?php echo __( 'Search', Kanban::$instance->settings->file ) ?>">
		...
		<a href="/wp-admin/admin.php?page=<?php echo Kanban::$instance->settings->basename ?>" class="btn btn-default"
		   target="_blank">
...
<?php wp_nonce_field( sprintf( '%s-save', Kanban::$instance->settings->basename ), Kanban_Utils::get_nonce() ); ?>
	...
	<style>
		.col_percent_w {
			width: <?php echo $wp_query->query_vars['kanban']->board->col_percent_w ?>%
		}

		.sidebar_w {
			width: <?php echo $wp_query->query_vars['kanban']->board->sidebar_w ?>%
		}
