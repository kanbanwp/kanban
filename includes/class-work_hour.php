...
class Kanban_Work_Hour
{
	...
	{
		if (  !isset( $_POST[Kanban_Utils::get_nonce()] ) || ! wp_verify_nonce( $_POST[Kanban_Utils::get_nonce()], sprintf('%s-save', Kanban::$instance->settings->basename)) || !isset($_POST[Kanban_Task::get_slug()]) || !is_user_logged_in() ) wp_send_json_error();
		...
		$post_data = array(
			...
			'post_title' => sanitize_text_field($_POST[Kanban_Task::get_slug()]['post_title']),
			'postmeta' => array(),
			'terms' => array()
		);
		...
		if ( $_POST[Kanban_Task::get_slug()]['postmeta'][$task_user_id_assigned_to] > 0 )
		{
			$post_data['post_author'] = $_POST[Kanban_Task::get_slug()]['postmeta'][$task_user_id_assigned_to];
		}
		...
		$post_data['postmeta'][$hour_task_id] = $_POST[Kanban_Task::get_slug()]['ID'];
		...
		$post_data['postmeta'][$hour_project_id] = $_POST[Kanban_Task::get_slug()]['postmeta'][$task_project_id];
		...
		$post_data['postmeta'][$hour_status_id] = $_POST[Kanban_Task::get_slug()]['terms'][$task_status][0];
